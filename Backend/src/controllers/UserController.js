    ...
    db.query("SELECT * FROM user WHERE Email = ?", [Email], (err, result) => {
      if (err) {
        return res.status(500).json({
          error: "Erreur lors de la vérification de l'email: " + err.message,
        });
      }
  ...
  loginUser: (req, res) => {
    ...
    db.query(sql, [email], (err, result) => {
      if (err) {
        res
          .status(500)
          .json({ error: "Erreur lors de la connexion: " + err.message });
      } else {
        if (
          result.length > 0 &&
          bcrypt.compareSync(password, result[0].PasswordHash)
        ) {
          const token = jwt.sign(
            { id: result[0].User_ID },
            "maSuperCleSecrete2023!#",
            {
    ...
    db.query(
      ...
      (err, result) => {
        if (err) {
          res
            .status(500)
            .send("Erreur lors de la mise à jour de l'utilisateur: " + err);
        } else {
  ...
  deleteUser: (req, res) => {
    ...
    db.query(sql, [userId], (err, result) => {
      if (err) {
        res
          .status(500)
          .send("Erreur lors de la suppression de l'utilisateur: " + err);
      } else {
